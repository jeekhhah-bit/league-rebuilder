...
export function MatchForm({ open, onOpenChange }: MatchFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (matchNumber > settings.maxMatches) {
      toast.error(`League is complete! All ${settings.maxMatches} matches have been played.`);
      return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-card border-border max-w-md mx-4">
        <DialogHeader>
          <DialogTitle className="font-display text-xl">Record Match {matchNumber}/{settings.maxMatches}</DialogTitle>
        </DialogHeader>
