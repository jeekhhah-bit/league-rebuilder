...
export function MatchHistory({ matches: propMatches, teams: propTeams }: MatchHistoryProps) {
  const { matches: storeMatches, teams: storeTeams } = useLeagueStore();
          ...
          <div className="min-w-[320px]">
            {recentMatches.map((match, index) => {
              ...
              return (
                <div
                  key={match.id}
                  className="flex items-center gap-2 md:gap-4 p-2 md:p-3 rounded-lg bg-muted/20 hover:bg-muted/30 transition-colors mb-2"
                >
                  ...
                  <div className="flex-1 flex items-center justify-between min-w-0">
                    <div className="flex items-center gap-1 md:gap-2 min-w-0 flex-1">
                      ...
                      <span className={cn('font-medium text-xs md:text-sm truncate', homeWin ? 'text-emerald-400' : 'text-foreground')}>
                        {homeTeam?.name}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1 md:gap-2 min-w-0 flex-1 justify-end">
                      <span className={cn('font-medium text-xs md:text-sm text-right truncate', awayWin ? 'text-emerald-400' : 'text-foreground')}>
                        {awayTeam?.name}
                      </span>
